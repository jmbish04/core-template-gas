...
import {detectAffectedWorkerProjects, parseArgs, run} from './lib/projects.mjs';
...
export async function getAffectedWorkerProjects({base, head}) {
  const changedFiles = await getChangedFiles(base, head);
  const projects = await detectAffectedWorkerProjects(changedFiles);
  return {changedFiles, projects};
}

async function main() {
  const args = parseArgs(process.argv.slice(2));
  const payload = await getAffectedWorkerProjects({
    base: args.base,
    head: args.head
  });
