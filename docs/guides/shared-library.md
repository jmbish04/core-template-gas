...
- `shared/src/workspace`: rich service classes for Calendar, Docs, Drive, Forms, Gmail, Sheets, Slides, sidebars, and web app responses
...
- `shared/src/ai`: provider-neutral AI access, prompt bases, schema-aware tool definitions, and the category-based tool registry
- `shared/src/agentic`: reusable planning, orchestration, execution summarization, and knowledge crystallization helpers
- `shared/src/tools`: modular one-file-per-tool definitions layered on top of the workspace service classes
...
- Add detailed JSDoc to new shared functions and classes so project teams can import them without reverse-engineering intent.
- Prefer adding a new `shared/src/tools/<domain>/<tool>.ts` file over embedding ad hoc tool objects inline in project code.
